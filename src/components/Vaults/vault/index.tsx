...
type props = {
  ...
  updateInitVaultData: (data: VaultInitType) => void;
  goBack: () => void;
};

const Vault = ({ currentAddress, vaultInitData, updateInitVaultData, goBack }: props) => {
  ...
  const {
    ...
    vaultInitData.collateralSymbol,
    vaultInitData.isHardVault
  );
  ...
  const [text, setText] = useState(t("vault.create-text", { asset: vaultInitData.assetSymbol }));
  ...
  async function loadVault() {
    ...
    if (currentVaultData.vaultId !== "0") {
      // @ts-ignore
      const vault = await currentVault.vaults(BigNumber.from(currentVaultData.vaultId));
      currentVaultData = {
        vaultId: currentVaultData.vaultId,
        collateral: vault.Collateral,
        debt: vault.Debt,
      };
    }

    if (currentVaultData.collateralSymbol !== "ETH") {
      // @ts-ignore
  ...
  useEffect(
    ...
    [currentAddress, currentCollateral, vaultInitData]
  );
  ...
  const isGasAsset = () =>
    (!isPolygon(currentNetwork.chainId) && vaultInitData.collateralSymbol === "ETH") ||
    (isPolygon(currentNetwork.chainId) && vaultInitData.collateralSymbol === "MATIC");
  ...
  const changeVault = async (newRatio: number, reset = false) => {
    const safeValue = vaultInitData.isHardVault ? 20 : 50;
    const warnValue = vaultInitData.isHardVault ? 10 : 30;
    let r = newRatio;
  ...
  const addCollateral = async () => {
    if (addCollateralTxt && parseFloat(addCollateralTxt) > 0) {
      ...
      try {
        if (isGasAsset()) {
          let tx;
          if (vaultInitData.collateralSymbol === "ETH") {
  ...
  const maxBurnTCAP = async (e: React.MouseEvent) => {
    ...
    const currentVaultDebtCall = await currentVaultRead?.vaults(vaultInitData.vaultId);
    ...
    <div className="dd-collateral">
      ...
      <Dropdown>
        <Dropdown.Toggle variant="secondary" id="dropdown-filters" className="text-left">
          <div className="collateral-toggle">
            <TokenIcon name={vaultInitData.assetSymbol} />
            <span>TCAP</span>
          </div>
        </Dropdown.Toggle>
        <Dropdown.Menu>
  ...
  const CollateralDropdown = () => (
    <div className="dd-collateral">
      ...
      <Dropdown onSelect={(eventKey) => handleTokenChange(eventKey || "ETH")}>
        <Dropdown.Toggle variant="secondary" id="dropdown-filters" className="text-left">
          <div className="collateral-toggle">
            <TokenIcon name={vaultInitData.collateralSymbol} />
            <span>{vaultInitData.collateralSymbol.toUpperCase()}</span>
          </div>
        </Dropdown.Toggle>
        <Dropdown.Menu>
